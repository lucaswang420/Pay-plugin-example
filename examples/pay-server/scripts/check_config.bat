...
REM Validates examples/pay-server/config.json for missing or placeholder values
...
set CONFIG_FILE=examples\pay-server\config.json
set ISSUES_FOUND=0
...
if not exist "examples\pay-server\certs\" (
    echo [ERROR] Certificate directory not found
    echo    Expected path: examples\pay-server\certs\
    set /a ISSUES_FOUND+=1
) else (
    echo [PASS] Certificate directory exists
    
    if not exist "examples\pay-server\certs\apiclient_key.pem" (
    ...
    if not exist "examples\pay-server\certs\wechatpay_platform.pem" (
