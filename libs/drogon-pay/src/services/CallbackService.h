...
// SPI whitelist exception: callback verification/decryption relies on
// WeChat-specific capabilities (decryptResource/getAppId/getMchId/5-arg
// verifyCallback) that are intentionally NOT part of the PaymentChannel SPI.
// The concrete client is obtained via dynamic_pointer_cast in the ctor.
...
class CallbackService
{
  public:
    ...
    // `wechatChannel` must be (or wrap) a WechatPayClient; the ctor downcasts
    // to access WeChat-specific decrypt/verify capabilities (see include note).
    CallbackService(
